                    ...
                    <table class="table">
                        ...
                        @foreach($wattage as $w)
                            <tr>
                                ...
                                <td>{{$w -> description}}</td>
                                <td>
                                    <a href="{{route('wattage_edit',$w -> id)}}" class="btn btn-outline-info"><i
                                            class="bx bx-edit-alt me-1"></i>Edit</a>
                                    <br><br>
                                    <form id="delete-form" action="{{ route('wattage.destroy', $w->id) }}"
                                          method="POST"
                                          style="display: inline-block;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-danger" data-bs-toggle="modal"
                                                data-bs-target="#delete{{$w->id}}">Xoá
                                        </button>
                                        <!-- Modal -->
                                        <div class="modal fade" id="delete{{$w->id}}" tabindex="-1"
